export default class BattleEventManagerDOM {
  ...
  getBattlerSettingHTML(settingName) {
    ...
    const { battlerList: savedBattlerList = [] } = this.app.storage.savedData;
    const existSaved = savedBattlerList?.some((b) => b);

    // 欠けている分は空オブジェクトで補完
    const battlers = existSaved ? [...savedBattlerList, ...Array(defaultBattlerLength - savedBattlerList.length).fill({})] : Array(defaultBattlerLength).fill({});

    const createBattlerItem = (battler, index) => `
    <li class="--battler">
      <span class="--index">${index + 1}</span>
      <input type="text" name="battler-name" value="${battler?.name ?? ''}">
      <input type="text" name="battler-desc" value="${battler?.desc ?? ''}">
      <input type="text" name="battler-info" value="${battler?.info ?? ''}">
      <button class="${settingName}__battler-delete"></button>
    </li>
  `;

    const battlerFlamesHTML = battlers.slice(0, defaultBattlerLength).map(createBattlerItem).join('');
